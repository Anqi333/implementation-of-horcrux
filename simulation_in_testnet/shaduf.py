...
def multi_work(method, mode, skew_param):
    ...
    for i in range(tx_load):
        ...
        total_tx_amount += amt
    total_channel_depletion_number = network_test()
    # result = total_channel_depletion_number / G.size()
    return success_tx_number/total_tx_number, success_tx_amount, total_tx_amount, total_channel_depletion_number


def multi_uniform_capacity(capacity, method, bind_mode):
    results = []
    repeat_time = 20
    ...
    print('average multi_success ratio:', ave_result[0])
    print('average multi_success volume:', ave_result[1])
    print('average multi_total volume:', ave_result[2])
...
def test_multi_uniform(capacity):
    ...
    # print('LN result for running 200 times with capacity 8:')
    # multi_uniform_capacity(method="LN", bind_mode=None, capacity=8)

    # print('Shaduf result for running 200 times with capacity 8:')
    # multi_uniform_capacity(method="Shaduf", bind_mode='high-to-low', capacity=8)

    for obj in capacity:
        print('capacity factor:', obj)

        # print('LN result:')
        # multi_uniform_capacity(method="LN", bind_mode=None, capacity=obj)

        print('HL-Shaduf result:')
        multi_uniform_capacity(method="Shaduf", bind_mode='high-to-low', capacity=obj)
