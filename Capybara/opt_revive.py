...
def network_test():
    ...
    for node in znode:
        for neighbor in G[node]:
            if balance_dict[(node, neighbor)] == 0:
...
def multi_work(mode, method, skew_param):
    global tx_8
    global revive_total
    global revive_success
...
def test_multi_uniform(capacity):
    ...
    # print('Revive-OPT result for running 200 times with capacity 8:')
    # multi_uniform_capacity(method="OPT-Revive", capacity=8)

    for obj in capacity:
        print('capacity factor', obj)
        print('OPT-Revive result:')
        multi_uniform_capacity(method="OPT-Revive", capacity=obj)
