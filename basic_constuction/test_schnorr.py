...
import hashlib
import sig_schnorr
from sig_schnorr import multi_sign_schnorr
...
def get_txf(input0: TxInput, input1: TxInput, sk_2pc_ai: SigningKey, sk_2pc_ib: SigningKey, c: float,
    ...
    sig_0 = multi_sign_schnorr(sk_2pc_ai, tx_id.encode())
    sig_1 = multi_sign_schnorr(sk_2pc_ib, tx_id.encode())
...
def get_txc(tx_in: TxInput, sk_3pc: SigningKey, pk_2pc_ai: VerifyingKey, pk_2pc_ib: VerifyingKey,
    ...
    sig = multi_sign_schnorr(sk_3pc, tx_id.encode())
    return tx_id, tx, sig
...
def update_channel(tx_in: TxInput, pk1: VerifyingKey, pk2: VerifyingKey, sk_2pc: SigningKey, c1: float, c2: float):
    ...
    sig = multi_sign_schnorr(sk_2pc, tx_id.encode())
    return tx_id, tx, sig
...
def gen_st_vc(flag_direction: int, c: float, c_sum: float, sk1: SigningKey, sk2: SigningKey):
    ...
    sig0_st_vc = sig_schnorr.sign_message_schnorr(sk1, hash_st_vc.encode())
    sig1_st_vc = sig_schnorr.sign_message_schnorr(sk2, hash_st_vc.encode())
...
print('Experiment based on Schnorr')
print('------------------------------------')
# private account
sk_a, vk_a = sig_schnorr.generate_keypair_schnorr()
str_a = f"{vk_a.to_string()}".encode()
sk_i, vk_i = sig_schnorr.generate_keypair_schnorr()
sk_b, vk_b = sig_schnorr.generate_keypair_schnorr()


# 2pc account for A,I and I,B
sk_ai, vk_ai = sig_schnorr.open_2pc_account_schnorr()
sk_ib, vk_ib = sig_schnorr.open_2pc_account_schnorr()
...
sk_3pc_i, vk_3pc_i = sig_schnorr.open_3pc_account_schnorr()
